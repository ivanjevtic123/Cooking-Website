$(document).ready(function() { 
    ...
    if(mealsPom == null) { //nema deserata:
        ...
        for(let i = 0; i < mealsPom.length; i++) {
            ...
            ).append("<br>").append("<button class='btn btn-secondary' id='"+mealsPom[i].id+"'>Go to a recepie</button>");
        }
    }
        ...
        if(searchText == '') { //unet prazan text u search => prikazi sve
            for(let i = 0; i < meals_search.length; i++) {
                if (meals_search[i].grupa == "predjelo") {
                    ...
                    ).append("<br>").append("<button class='btn btn-secondary' id='"+meals_search[i].id+"' >Go to a recepie</button>");
                }
            }
            return;
        }
