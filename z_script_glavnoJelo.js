$(document).ready(function() { 
    ...
    for(let i = 0; i < mealsArr_LS.length; i++) {
        if(mealsArr_LS[i].grupa == "glavnoJelo") {
            mealsPom.push(mealsArr_LS[i]);
        }
    }

    if(mealsPom == null) { //nema deserata:
        $("#recepti").append("<h2>"+"No main courses are available"+"</h2>").append("<hr>")
    } else { //ima deserata:
    ...
    function myFunction() {
        ...
        if(searchText == '') { //unet prazan text u search => prikazi sve
            for(let i = 0; i < meals_search.length; i++) {
                if (meals_search[i].grupa == "glavnoJelo") {
                    $("#recepti").append("<hr>").append(
        ...
        //unet text:
        for (let i = 0; i < meals_search.length; i++) {
            if (meals_search[i].grupa == "glavnoJelo" && meals_search[i].ime.includes(searchText)){
                $("#recepti").append("<hr>").append(
                    '<h2>'+meals_search[i].ime+'</h2>'
                ).append("<br>").append(
                    $("<img>").attr("src", meals_search[i].galerija)
    ...
    function sortMeals() {
        ...
        if(nacin == null || kriterijum == null) {
            alert("Select a sotring criteria!");
            return;
        }
        ...
        if(nacin == "rastuce") {
            for(let i = 0; i < meals_sorted.length; i++) {
                if (meals_sorted[i].grupa == "glavnoJelo"){
                    $("#recepti").append("<hr>").append(
    ...
    $('#serbian').bind('click', function(){
        window.location.href = "glavnoJelo.html";
        return false;
    });
